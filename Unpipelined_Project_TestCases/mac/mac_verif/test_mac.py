...
async def test_mac(dut):
    ...
    for i in range(0,1049):
        ...
        assert int(MAC_bin_int[i]) == int(str(dut.macop.value)), f'MAC Output Mismatch, Expected = {MAC_bin_int[i]} DUT = {int(str(dut.macop.value))}'
    ...
    for i in range(0,1000):
        ...
        await RisingEdge(dut.CLK)
        dut._log.info(f'output {dut.macop.value}')
        assert str(MAC_bin_fp[i])[0:30] == str(dut.macop.value)[0:30], f'MAC Output Mismatch, Expected = {str(MAC_bin_fp[i])} DUT = {str(dut.macop.value)}' 
