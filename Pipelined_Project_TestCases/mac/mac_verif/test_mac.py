...
async def test_mac(dut):
    ...
    for i in range(0,1054):
        ...
        if i>4:
            ...
            assert int(MAC_bin_int[i-5]) == int(str(dut.macop.value)), f'MAC Output Mismatch, Expected = {MAC_bin_int[i-4]} DUT = {int(str(dut.macop.value))}'
    ...
    for i in range(0,1000):
        ...
        if i>4:
            dut._log.info(f'output {dut.macop.value}')
            assert str(MAC_bin_fp[i-5])[0:30] == str(dut.macop.value)[0:30], f'MAC Output Mismatch, Expected = {str(MAC_bin_fp[i])} DUT = {str(dut.macop.value)}' 
